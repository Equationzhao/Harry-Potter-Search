...
class item{
public:
	item(int const &p_line,string const &p_chapter,string const &p_page) : line(p_line),page(p_page),
		chapter(p_chapter),p_line_(p_line),
		p_chapter_(p_chapter),
		p_page_(p_page)	{
		idGenerator++;
		id = idGenerator;
		if (line == -1){
...
private:
	...
	string book;
	int const p_line_;
	string const p_chapter_;
	string const p_page_;
	static bool found;
